...
def follow_instructions(input_columns, instructions): 
    ...
    for row in range(len(instructions)): # 1. řádek v délce 4
        ...
        for number_of_item in range(number_of_items): # kolikrát to udělá = počet beden
            selected_item = input_columns[from_column-1][-1]
            print('selected item:', selected_item)
            del input_columns[from_column-1][-1]
            input_columns[to_column-1].append(selected_item)
            print(input_columns, '\n')
    for i in range(len(input_columns)): 
        last_items.append(input_columns[i][-1])
    print('last: ', last_items)
    last_items = ''.join(last_items)
    print('last: ', last_items)     # BWNCQRMDB
